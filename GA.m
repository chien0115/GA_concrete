...
max_interrupt_time = [5, 5, 15]; % 工地最大容許中斷時間 (分鐘)
max_truck__interrupt_time=[3, 4, 3];%預拌車最大容許中斷時間 (分鐘)
demand_trips = [2, 2, 1]; % 各工地需求車次

penalty_site_value = 5; % 懲罰時間 (分鐘)
penalty_truck_value = 3;
...
for i = 1:tg
    % 初始化
    P = population(n, demand_trips); % 初始化種群
    K = zeros(tg, 2); % 儲存適應度的矩陣
    [x1, y1] = size(P);
    % 交配操作
    [E, dispatch_times] = evaluation(P, t, start_time, work_time, travel_time_to, travel_time_back, max_interrupt_time, max_truck__interrupt_time, demand_trips, penalty_site_value,penalty_truck_value); % 評估族群 P 中每個染色體的適應度
